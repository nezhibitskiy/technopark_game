...
bool StateStack::handleEvent(const sf::Event &event,std::queue<BaseMessage>* request) {



    for (auto itr = mStack.rbegin(); itr != mStack.rend(); ++itr) {
        return (*itr)->handleEvent(event,request);
...
void StateStack::ChangeState() {

    (*mStack.front()).ChangeState();
